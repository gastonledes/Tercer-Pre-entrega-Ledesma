...
def buscar(request):
    if request.GET['apellido']:
        ...
        categoria= Jugadores.objects.filter(apellido__icontains=apellido)
        numerocarnet =Jugadores.objects.filter(apellido__icontains=apellido)
        
        return render(request,'Inicio.html', {"nombre": nombre, "apellido": apellido, "categoria": categoria, "numerocarnet": numerocarnet})
    else:
...
def entrenadores(request):
    if request.method == 'POST':
        ...
        if miformulario.is_valid:
            ...
            jugador = Entrenadores(nombre=informacion['nombre'], apellido=informacion['apellido'], categoria_cargo= informacion['Categoria a cargo'])
            jugador.save()
